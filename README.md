...
- **Built-in logging and verbosity** — standard `logging` with Rich output, wired to `--verbose` / `-v`
...
- **Swagger/OpenAPI client generation** — `Cli.from_swagger("spec.json")` turns any OpenAPI 3.0 spec into a working CLI; path params become args, query params become options, HTTP calls via stdlib `urllib`
