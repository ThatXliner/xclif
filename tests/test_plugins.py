...
# ---------------------------------------------------------------------------
# find_path_plugin — Git-style PATH lookup (lazy)
# ---------------------------------------------------------------------------
...
def test_find_path_plugin_returns_none_when_not_found(tmp_path):
    """find_path_plugin returns None when the executable is not in PATH."""
    from xclif.plugins import find_path_plugin

    with patch.dict("os.environ", {"PATH": str(tmp_path)}):
        result = find_path_plugin("myapp", "nonexistent")

    assert result is None


def test_find_path_plugin_returns_none_wrong_prefix(tmp_path):
    """find_path_plugin ignores executables that don't match {root}-{cmd}."""
...
def test_find_path_plugin_handles_empty_path():
    """find_path_plugin returns None when PATH is unset."""
